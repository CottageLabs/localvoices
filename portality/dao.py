...
class ListSongsQuery(object):
    _base_query = {
        ...
        "size" : 50,
        "sort" : [{"order_by_title.exact" : {"order" : "asc"}}]
    }
    ...
    def query(self):
        ...
        if self.order.lower() in ["asc", "desc"]:
            q["sort"][0]["order_by_title.exact"]["order"] = self.order.lower()
        
        if self.initial_letters is not None:
            qs = deepcopy(self._query_string)
            qs["query_string"]["query"] = "order_by_title:" + self.initial_letters.lower() + "*"
            q["query"] = qs
